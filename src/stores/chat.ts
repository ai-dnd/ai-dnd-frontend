...
export const useChatStore = defineStore("chat", () => {
  ...
  const sendMessage = async (message: SendMessageParams) => {
    ...
    try {
      ...
      const optimisticUserMessage: MessageResponse = {
        ...
        status: message.status || 'completed', // 标记为待处理状态
      ...
      // if(response.status === 'need_react') {
      //   // 如果需要用户进行技能鉴定，标记为 need_react 状态
  ...
  const getUserMessagesBySession = async (sessionId: string): Promise<ChatMessage[]> => {
  ...
  return {
    ...
    retryMessage,
    removeMessage,
    fetchUserSessionsByDocumentId,
    getUserMessagesBySession,
  };
});
